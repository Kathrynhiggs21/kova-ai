...
def render_one(row: dict[str, str], output_dir: Path, status_path: Path, mode: str, image_size: str, retries: int) -> dict:
    card_id = field(row, "Card ID", "Card_ID", "id")
    ...
    if output_path.exists() and output_path.stat().st_size > 10_000:
        record = {"card_id": card_id, "subject": subject, "status": "completed", "skipped": True, "file": str(output_path), "bytes": output_path.stat().st_size, "rules": rules.to_dict()}
    ...
    for attempt in range(1, retries + 2):
        try:
            ...
            record = {"card_id": card_id, "subject": subject, "status": "completed", "file": str(output_path), "bytes": output_path.stat().st_size, "model": MODEL_NAME, "mode": mode, "requested_size": image_size, "final_pixel_size": list(final_size), "aspect_ratio": "5:7", "attempt": attempt, "rules": rules.to_dict()}
            append_jsonl(status_path, record)
            return record
    ...
    record = {"card_id": card_id, "subject": subject, "status": "failed", "model": MODEL_NAME, "mode": mode, "error": last_error, "rules": rules.to_dict()}
    append_jsonl(status_path, record)
    return record
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        with preview.open("w", encoding="utf-8") as handle:
            for row, prompt, rules in resolved:
                handle.write(json.dumps({"card_id": field(row, "Card ID", "Card_ID", "id"), "subject": field(row, "Subject", "Species", "species"), "prompt": prompt, "rules": rules}, ensure_ascii=False) + "\n")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.concurrency)) as pool:
        futures = [pool.submit(render_one, row, output_dir, status_path, args.mode, args.image_size, args.retries) for row in rows]
    ...
    summary = {"submitted": len(rows), "completed": completed, "failed": failed, "model": MODEL_NAME, "mode": args.mode, "aspect_ratio": "5:7", "files": [result.get("file") for result in results if result.get("file")]}
    (REPORTS_DIR / f"summary_{args.mode}.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
